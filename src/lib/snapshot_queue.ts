...
export class SidecarSnapshotQueueSaturatedError extends Error {
  statusCode: number;

  constructor(message: string) {
    super(message);
    this.name = "SidecarSnapshotQueueSaturatedError";
    this.statusCode = 503;
  }
}
...
export async function runWithSnapshotQueue<T>(task: QueueTask<T>): Promise<QueueRunResult<T>> {
  ...
  if (queue.length >= MAX_QUEUE) {
    throw new SidecarSnapshotQueueSaturatedError(
      `sidecar snapshot queue is full (${queue.length}/${MAX_QUEUE}); retry shortly`,
    );
  }
